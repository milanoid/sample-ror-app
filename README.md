...
### Run Tests
```bash
docker run --rm -v "$(pwd):/app" -w /app ruby:3.2 bash -c "bundle install && bin/rails test"
```

### Run Linting
```bash
# RuboCop
docker run --rm -v "$(pwd):/app" -w /app ruby:3.2 bash -c "bundle install && bundle exec rubocop"

# Brakeman security check
docker run --rm -v "$(pwd):/app" -w /app ruby:3.2 bash -c "bundle install && bundle exec brakeman --no-pager"
```

### Start the Server (Optional)
```bash
docker run --rm -v "$(pwd):/app" -w /app -p 3000:3000 ruby:3.2 bash -c "bundle install && bin/rails db:migrate && bin/rails server -b 0.0.0.0"
```
...
- **Model Tests**: Post model validations and functionality
- **Controller Tests**: All CRUD operations (index, show, new, create, edit, update, destroy)
- **System Tests**: Full-stack integration tests
